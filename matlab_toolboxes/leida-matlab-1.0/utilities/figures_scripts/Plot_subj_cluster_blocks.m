...
% Subject specific save directory
subj_dir = [save_dir '/' 'subject_data' '/'];
mkdir(subj_dir);
...
for i = 1:length(idx_data)
    ...
    if contains(Data_info(s).name,string(subj))
        ...
        writematrix(Ctime, fullfile(subj_dir, ['K' num2str(selectedK) '_cluster_timecourse_' char(subj) '.csv']))    
